...
def run_test(zoneid: str,
    ...
    if not (parent_directory_path / "Results").exists():
        (parent_directory_path / "Results").mkdir(parents=True, exist_ok=True)
    # Store all the query and responses in a json file
    with open(parent_directory_path / "Results" / (zoneid + '.json'), 'w') as all_res_fp:
