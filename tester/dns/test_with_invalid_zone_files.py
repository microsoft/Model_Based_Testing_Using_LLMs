...
def run_test(input_args: Namespace,
    ...
    print(f'+Containers prepared for zone {zoneid} in {parent_dir}')
    
    print(f'+Getting queries for zone {zoneid} in {parent_dir}...')
    queries = get_queries_invalid_zones(zoneid, total_impl_tested,
                                        parent_dir, zone_domain, logger)
    print(f'+Got {len(queries)} queries for zone {zoneid} in {parent_dir}.')
    if not queries:
        return
    differences = []
    results = []
    for query in queries:
        ...
        for impl, (check, port) in implementations.items():
            if check:
                print(f'--Querying {impl} for {qname} {qtype}...')
                respo = querier(qname, qtype, port * int(cid))
                print(f'--Got response from {impl} for {qname} {qtype}.')
                print(f'--Response: {respo}')
                if not isinstance(respo, dns.message.Message):
                    print(f'--{impl} did not respond properly for {qname} {qtype}')
                    # single_impl = {}
                    # single_impl[impl] = (True, port)
                    # print(f'--Re-preparing {impl}\'s container for zone {zoneid} in {parent_dir}...')
                    # prepare_containers(parent_dir / ZONE_FILES / (zoneid + '.txt'),
                    #                    zone_domain, cid, True, single_impl, tag)
                    # logger.write(f'{datetime.now()}\tRestarted {impl}\'s container while'
                    #              f' testing zone {zoneid}\n')
                    # time.sleep(1)
                    # respo = querier(qname, qtype, port * int(cid))
                    # print(f'--Got response from {impl} for {qname} {qtype} after restarting the container.')
                responses.append((impl, respo))

        # Store all the responses for the query
        results.append({"qname": qname, "qtype": qtype, "responses": [(impl, resp.to_text()) for impl, resp in responses]})
...
def run_tests_helper(input_args: Namespace,
    ...
    print(f"\n===Entered run_tests_helper with input_dir: {input_dir} ===")
    if not (input_dir.exists() or input_dir.is_dir()):
        print(f'The input path {input_dir} does not exist or is not a directory.')
        return
    zone_files_dir = input_dir / ZONE_FILES
    preprocessor_output_dir = input_dir / PREPROCESSOR_DIRECTORY
    print("Zone files directory:", zone_files_dir)
    print("Preprocessor output directory:", preprocessor_output_dir)
    print("Zone files directory exists:", zone_files_dir.exists(),
            "and is a directory:", zone_files_dir.is_dir())
    print("Preprocessor output directory exists:", preprocessor_output_dir.exists(),
            "and is a directory:", preprocessor_output_dir.is_dir())
    if zone_files_dir.exists() and zone_files_dir.is_dir() and \
        ...
        print(f'\n===Generating GRoot image===')
        generate_groot_image(logger)
        print(f'GRoot image generated successfully. Starting the GRoot container.')
        # Start the container for the GRoot
        delete_container('groot_server')
        cmd = "sudo docker run -d --name=groot_server groot:ferret"
        ...
        delete_container('groot_server')
        print(f'Containers for id {input_args.id} stopped and removed successfully.')
    else:
        # if input_dir.is_dir():
        #     for subdir in input_dir.iterdir():
        #         run_tests_helper(input_args, subdir, logger)
        print(f'The input path {input_dir} does not contain {ZONE_FILES} and '
              f'{PREPROCESSOR_DIRECTORY} directories.')
