import json
import glob
import os
import pandas as pd
from pandas.io.formats.style import Styler
from pybatfish.client.session import Session
from pybatfish.datamodel import *
from pybatfish.datamodel.answer import *
from pybatfish.datamodel.flow import *
from pybatfish.util import get_html

bf = Session(host="localhost")

SNAPSHOT_DIR = '../../snapshot'

preamble = """
!
version 15.2
service timestamps debug datetime msec
service timestamps log datetime msec
!
hostname R1
!
boot-start-marker
boot-end-marker
no aaa new-model
no ip icmp rate-limit unreachable
ip cef
no ip domain lookup
ip domain name lab.local
no ipv6 cef
multilink bundle-name authenticated
ip tcp synwait-time 5
!
"""

def  int2ip(ip):
    return '.'.join([str((ip >> i) & 0xff) for i in [24, 16, 8, 0]])

def prefixLengthToMask(prefixLength):
    mask = 0
    for i in range(prefixLength):
        mask |= (1 << (31 - i))
    return mask

def masktoOctet(mask):
    mask = prefixLengthToMask(mask)
    return '.'.join([str((mask >> i) & 0xff) for i in [24, 16, 8, 0]])


def run_batfish_example(route):
    NETWORK_NAME = 'example'
    SNAPSHOT_NAME = 'example_snapshot'
    SNAPSHOT_PATH = '../../snapshot'

    bf.set_network(NETWORK_NAME)
    bf.init_snapshot(SNAPSHOT_PATH, name=SNAPSHOT_NAME, overwrite=True)
    
    # df = bf.q.initIssues().answer().frame()
    # print(df)
    
    # df = bf.q.bgpSessionStatus(nodes='R1').answer().frame()
    # print(df)
    
    # df = bf.q.bgpSessionStatus(nodes='R2').answer().frame()
    # print(df)
    df1 = bf.q.bgpRib(nodes='R1').answer().frame()
    df2 = bf.q.bgpRib(nodes='R2').answer().frame()
    df3 = bf.q.bgpRib(nodes='R3').answer().frame()
    
    newdf = (pd.concat([df1, df2, df3])).reset_index(drop=True)
    
    prefix_uint = route['prefix'] & prefixLengthToMask(route['prefixLength'])
    prefix = int2ip(prefix_uint) + '/' + str(route['prefixLength'])
    
    # print(newdf)
    newdf = newdf[newdf['Network'] == prefix][["Node", "Local_Pref", "Metric", "Communities"]].reset_index(drop=True)
    # print(newdf)
    return newdf


def constructPrefixList(pl):
    prefix_list_lines = []
    
    for i in range(len(pl)):
        if pl[i]['any']:
            if pl[i]['permit']:
                prefix_list_lines.append('ip prefix-list PL seq ' + str((i+1)*10)  + ' permit any')
            else:
                prefix_list_lines.append('ip prefix-list PL seq ' + str((i+1)*10)  + ' deny any')
        else:
            prefix = int2ip(pl[i]['prefix']) + '/' + str(pl[i]['prefixLength'])
            if pl[i]['permit']:
                prefix_list_lines.append('ip prefix-list PL seq ' + str((i+1)*10)  + ' permit ' + prefix)
            else:
                prefix_list_lines.append('ip prefix-list PL seq ' + str((i+1)*10)  + ' deny ' + prefix)

    return prefix_list_lines

def write_config(router1, router2, router3, route, rmap):
    router1_config_lines = []
    router2_config_lines = []
    router3_config_lines = []
    
    preamble_lines = preamble.split('\n')
    
    router1_config_lines.extend(preamble_lines)
    router2_config_lines.extend(preamble_lines)
    router3_config_lines.extend(preamble_lines)
    
    router2_config_lines[6] = 'hostname R2'
    router3_config_lines[6] = 'hostname R3'
    
    ############################### ROUTER 1 CONFIGURATION ####################
    router1_config_lines.extend([
        'interface eth0/1',
        ' ip address 3.0.0.3 255.0.0.0',
        'interface eth0/2',
        ' ip address ' + int2ip(route['prefix']) + ' ' + masktoOctet(route['prefixLength']),
        '',
        'route-map RMAP permit 10',
        ' set metric ' + str(route['med']),
        ' set local-preference ' + str(route['localPref']),
        # ' set ip next-hop ' + int2ip(route['nextHop']),
        'router bgp ' + str(router1['asNumber']),
        '',
        ' neighbor 3.0.0.2 remote-as ' + str(router2['asNumber']),
        ' neighbor 3.0.0.2 route-map RMAP out',
        ' neighbor 100.0.0.2 remote-as 65000',
        ' network ' + int2ip(route['prefix']) + '/' + str(route['prefixLength']),
        ' network 3.0.0.0'
    ])
    
    if router1['isRR']:
        router1_config_lines.append(' neighbor 3.0.0.2 route-reflector-client')
        
    ############################### ROUTER 2 CONFIGURATION ####################
    route_map_lines = constructPrefixList(rmap['matchPrefixList'])

    route_map_lines.extend([
        'route-map RMAP permit 10',
        ' match ip address prefix-list PL',
        ' match local-preference ' + str(rmap['matchLocalPref']),
        ' match metric ' + str(rmap['matchMed']),
        ' set local-preference ' + str(rmap['setLocalPref']),
        ' set metric ' + str(rmap['setMed']),
        # ' set ip next-hop ' + int2ip(rmap['setNextHop']),
        ' set community ' + str(rmap['setCommunity']) + ' additive'
    ])
    
    router2_config_lines.extend([
        'interface eth0/1',
        ' ip address 3.0.0.2 255.0.0.0',
        'interface eth0/2',
        ' ip address 4.0.0.2 255.0.0.0',
        '',
    ])
        
    router2_config_lines.extend(route_map_lines)
    
    router2_config_lines.extend([
        'router bgp ' + str(router2['asNumber']),
        ' bgp always-compare-med',
        ' bgp send-community both',
        ' neighbor 3.0.0.3 remote-as ' + str(router1['asNumber']),
        ' neighbor 4.0.0.3 remote-as ' + str(router3['asNumber']),
        ' neighbor 4.0.0.3 route-map RMAP out',
        ' network 3.0.0.0',
        ' network 4.0.0.0'
    ])
    
    if router2['isRR1']:
        router2_config_lines.append(' neighbor 3.0.0.3 route-reflector-client')
        
    if router2['isRR3']:
        router2_config_lines.append(' neighbor 4.0.0.3 route-reflector-client')
        
    ############################### ROUTER 3 CONFIGURATION ####################
    router3_config_lines.extend([
        'interface eth0/1',
        ' ip address 4.0.0.3 255.0.0.0',
        '',
        'router bgp ' + str(router3['asNumber']),
        ' neighbor 4.0.0.2 remote-as ' + str(router2['asNumber']),
        ' network 4.0.0.0'
    ])
    
    if router3['isRR']:
        router3_config_lines.append(' neighbor 4.0.0.2 route-reflector-client')
        
    if not os.path.exists(SNAPSHOT_DIR):
        os.mkdir(SNAPSHOT_DIR)
        os.mkdir(SNAPSHOT_DIR + '/configs')
        
    with open(SNAPSHOT_DIR + '/configs/R1.cfg', 'w') as f:
        f.write('\n'.join(router1_config_lines))
    
    with open(SNAPSHOT_DIR + '/configs/R2.cfg', 'w') as f:
        f.write('\n'.join(router2_config_lines))
        
    with open(SNAPSHOT_DIR + '/configs/R3.cfg', 'w') as f:
        f.write('\n'.join(router3_config_lines))
    
    
with open('../../../rr1_rmap_all_test_cases.json', 'r') as f:
    test_cases = json.load(f)
    
# f = open('results.txt', 'w')

# i = 30
# t = test_cases[i]

if not os.path.exists('results'):
    os.mkdir('results')
    
    
for i in range(len(test_cases)):
    t = test_cases[i]
    print("Test case:", (i+1))

    route = t[0]
    rmap = t[1]
    p_inRRflag, p_outRRflag, p_inAS, p_outAS = t[2], t[3], t[4], t[5]

    router2_asnum = 200
    if not p_inAS:
        router1_asnum = 100
    else:
        router1_asnum = 200
        
    if not p_outAS:
        router3_asnum = 300
    else:
        router3_asnum = 200
        
    # isRR<num> means route reflector to the neighbor
    # isClient<num> means route reflector client to the neighbor

    if p_inRRflag == 0:
        isRR1 = False
        isClient1 = True
        isRR_router1 = True
        isClient_router1 = False
    elif p_inRRflag == 1:
        isRR1 = True
        isClient1 = False
        isRR_router1 = False
        isClient_router1 = True
    else:
        isRR1 = False
        isClient1 = False
        isRR_router1 = False
        isClient_router1 = False
        
    if p_outRRflag == 0:
        isRR3 = False
        isClient3 = True
        isRR_router3 = True
        isClient_router3 = False
    elif p_outRRflag == 1:
        isRR3 = True
        isClient3 = False
        isRR_router3 = False
        isClient_router3 = True
    else:
        isRR3 = False
        isClient3 = False
        isRR_router3 = False
        isClient_router3 = False
        
    router1 = {
        'asNumber': router1_asnum,
        'isRR': isRR_router1,
        'isClient': isClient_router1
    }

    router2 = {
        'asNumber': router2_asnum,
        'isRR1': isRR1,
        'isClient1': isClient1,
        'isRR3': isRR3,
        'isClient3': isClient3
    }

    router3 = {
        'asNumber': router3_asnum,
        'isRR': isRR_router3,
        'isClient': isClient_router3
    }

    write_config(router1, router2, router3, route, rmap['stanza'][0])
    new_df = run_batfish_example(route)
    print(new_df)

    with open('results/test_case' + str(i) + '.txt', 'w') as f:
        f.write(str(new_df))

# f.close()